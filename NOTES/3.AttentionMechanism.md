...
NOTE: Attention score will be high when we multiply a word's own query vector with its own key vector, since it needs to retain information
about itself, not just how it interacts with other words.
...
Generate Key Matrices (K): Multiply each word's embedding vector by the learnable weight matrix W_K to produce key vectors.

Generate Query Matrices (Q): Multiply each word's embedding vector by the learnable weight matrix W_Q to produce query vectors.

Compute Attention Scores: Perform a dot product of each query vector with all key vectors in the sequence to generate attention scores.

Interpret Scores: Apply scaling and softmax to attention scores. Higher attention values indicate which words are more relevant to the current word.